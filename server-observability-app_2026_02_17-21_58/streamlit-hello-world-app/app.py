...
with tabs[2]:
    ...
    # Pull top wait type for context
    if wait_type_col and wait_time_col:
        top_wait_df = run_query(f"""
        SELECT
          {q_ident(wait_type_col)} AS wait_type,
          SUM(CAST({q_ident(wait_time_col)} AS DOUBLE)) AS total_wait_ms
        FROM {fqtn(CATALOG, SCHEMA, PERF_TABLE)}
        ...
        LIMIT 1
        """)
        if not top_wait_df.empty and "wait_type" in top_wait_df.columns:
            top_wait = str(top_wait_df.iloc[0]["wait_type"])
            recs.append(
