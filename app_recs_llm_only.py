...
def generate_llm_recommendations(
    ...
    *,
    ...
    user_msg = (
        "You MUST base your advice on the provided telemetry JSON. "
        "Your recommendations MUST be explicitly tied to the most_expensive_queries_examples list. "
        "Output format:\n"
        "1) A short heading line: 'Recommendations tied to most expensive queries'\n"
        "2) Then for EACH query example provided, write:\n"
        "   - Query label: CPU or Physical Reads (based on which field is populated)\n"
        "   - A brief 'Why it is expensive' based only on the evidence in the snapshot (or say 'insufficient data')\n"
        "   - 2-4 concrete tuning actions (indexes, predicates, plan shape, parameterization, stats, etc.)\n"
        "3) Then optionally add 1-2 server-level actions if supported by waits/memory/cache values.\n"
        "Do not invent missing query text, indexes, wait causes, or metrics."
        f"\n\nTelemetry (JSON): {context}"
    )
...
with tabs[2]:
    ...
    # Show which endpoint is used (so it's obvious this is LLM-driven)
    ...
    # If we don't have any expensive queries, there's nothing meaningful to recommend on.
    if ("meq_df" not in locals()) or (not isinstance(meq_df, pd.DataFrame)) or meq_df.empty:
        st.info("No expensive-query rows found for the selected server, so LLM recommendations cannot be generated.")
    else:
        # Generate LLM recommendations that are explicitly tied to the *most expensive queries*.
        # NOTE: We intentionally do NOT auto-fallback to rule-based text because you want
        # recommendations that are actually produced by the LLM.
        ...
        # Render in an auto-sizing box (no fixed height)
        html_recs = llm_text.replace("\n", "<br>")
        st.markdown(
            f"""
<div style="
